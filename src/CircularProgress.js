...
export default class CircularProgress extends React.PureComponent {
  ...
  backgroundCirclePath(x, y, radius, startAngle, endAngle){
    var start = this.polarToCartesian(x, y, radius, endAngle * 0.9999);
    var end = this.polarToCartesian(x, y, radius, startAngle);
    var largeArcFlag = endAngle - startAngle <= 180 ? '0' : '1';
    var d = [
  ...
  maxFill = 80
  clampFill = fill => Math.min(this.maxFill, Math.max(0, fill));
  ...
  render() {
    ...
    const backgroundCirclePath = this.backgroundCirclePath(size / 2, size / 2, size / 2 - width / 2, 0, arcSweepAngle * this.maxFill / 100);
    //backgroundCirclePath
